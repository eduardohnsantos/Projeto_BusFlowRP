...
if not os.path.exists(caminho_mestre):
    raise FileNotFoundError(
        "❌ Execute primeiro o script 'listar_linhas.py' para gerar a lista mestre!"
    )
...
linhas = (
    dados_mestre if isinstance(dados_mestre, list) else dados_mestre.get("routes", [])
)
...
for idx, linha in enumerate(linhas, 1):
    ...
    print(
        f"[{idx}/{len(linhas)}] 🚌 Baixando horários da linha {short_name} - {long_name} (ID: {route_id})..."
    )
    ...
    try:
        ...
        if resposta.status_code == 200:
            ...
            print(
                f"⚠️ Servidor respondeu com status {resposta.status_code} para a linha {short_name}"
            )
